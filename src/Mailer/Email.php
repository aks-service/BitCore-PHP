...
class Email implements JsonSerializable, Serializable
{
     ...
     *
     * @var \Bit\Mailer\AbstractTransport
     */
    ...
    protected static $_dsnClassMap = [
        'debug' => 'Bit\Mailer\Transport\DebugTransport',
        'mail' => 'Bit\Mailer\Transport\MailTransport',
        'smtp' => 'Bit\Mailer\Transport\SmtpTransport',
    ];
    ...
    public function getHeaders(array $include = [])
    {
        ...
        if ($this->_messageId !== false) {
            if ($this->_messageId === true) {
                $headers['Message-ID'] = '<' . str_replace('-', '', UUID::v4()) . '@' . $this->_domain . '>';
            } else {
     ...
     *
     * @param string|\Bit\Mailer\AbstractTransport|null $name Either the name of a configured
     *   transport, or a transport instance.
     * @return \Bit\Mailer\AbstractTransport|$this
     ...
     * @return \Bit\Mailer\AbstractTransport
    ...
    protected function _constructTransport($name)
    {
        ...
        $className = Bit::className($config['className'], 'Mailer/Transport', 'Transport');
        if (!$className) {
            $className = Bit::className($config['className'], 'Network/Email', 'Transport');
            trigger_error(
     ...
     * @return $this
     * @throws \InvalidArgumentException
     * @see \Bit\Mailer\Email::attachments()
     */
     ...
     * @param array|\Bit\Mailer\AbstractTransport|null $config Either an array of configuration
    ...
    /**
     * Static method to fast create an instance of \Bit\Mailer\Email
     *
     * @param string|array|null $to Address to send (see Bit\Mailer\Email::to()). If null, will try to use 'to' from transport config
     ...
     * @return \Bit\Mailer\Email Instance of Bit\Mailer\Email
     * @throws \InvalidArgumentException
     */
     ...
     * @return \Bit\Mailer\Email Configured email instance.
     */
