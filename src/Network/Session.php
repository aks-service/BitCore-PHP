...
class Session
{
     ...
     * - cache: Use the BitPHP caching system as an storage for the session, you will need
     *   to pass the `config` key with the name of an already configured Cache engine.
     * - database: Use the BitPHP ORM to persist and manage sessions. By default this requires
     *   a table in your database named `sessions` or a `model` key in the configuration
     *   to indicate which Table object to use.
     * - bit: Use files for storing the sessions, but let BitPHP manage them and decide
     *   where to store them.
     *
     * The full list of options follows:
     *
     * - defaults: either 'php', 'database', 'cache' or 'bit' as explained above.
     ...
     * @see \Bit\Network\Session::__construct()
     */
    ...
    public function engine($class = null, array $options = [])
    {
        ...
        $className = Bit::className($class, 'Network/Session');
        if (!$className) {
