...
class Plugin
{
     ...
     * @throws \Bit\Core\Exception\MissingPluginException if the folder for the plugin to be loaded is not found
     * @return void
     */
    ...
    protected static function _loadConfig()
    {
        ...
        $vendorFile = dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'bit-plugins.php';
        if (!file_exists($vendorFile)) {
            $vendorFile = dirname(dirname(dirname(dirname(__DIR__)))) . DIRECTORY_SEPARATOR . 'bit-plugins.php';
            if (!file_exists($vendorFile)) {
    ...
    public static function loadAll(array $options = [])
    {
        static::_loadConfig();
        $plugins = [];
        foreach (Bit::path('Plugin') as $path) {
            if (!is_dir($path)) {
                continue;
            }
     ...
     * @return mixed
     * @see \Bit\Core\Plugin::load() for examples of bootstrap configuration
     */
