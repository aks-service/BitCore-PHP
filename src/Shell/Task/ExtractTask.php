...
class ExtractTask extends Shell
{
    ...
    /**
     * Holds whether this call should extract the BitPHP Lib messages
     *
     * @var bool
     */
    ...
    public function main()
    {
        ...
        if (isset($this->params['extract-core'])) {
            ...
            $response = $this->in('Would you like to extract the messages from the BitPHP core?', ['y', 'n'], 'n');
            $this->_extractCore = strtolower($response) === 'y';
        }

        if (!empty($this->params['exclude-plugins']) && $this->_isExtractingApp()) {
            $this->_exclude = array_merge($this->_exclude, Bit::path('Plugin'));
        }
    ...
    public function getOptionParser()
    {
        ...
        $parser->description(
            'BitPHP Language String Extraction:'
        )->addOption('app', [
            ...
            'help' => 'Extract messages from the BitPHP core libs.',
            'choices' => ['yes', 'no']
    ...
    protected function _extractTokens()
    {
        ...
        $isPHP = function($file) use ($isVerbose,$progress) {
        ...
        $isHTML = function($file) use ($isVerbose,$progress) {
            $content = file_get_contents($file);
            if(!$content)
                return;
            $html = new QueryObject($content);

            $html->find('[data-speak]')->each(function (QueryObject $obj){
                $details = [
                    'file' => $this->_file,
                    'line' => $obj->nodes()[0]->getLineNo(),
                ];
                $domain = $obj->attr('data-speak-domain') ? : 'default';
                $singular = null;

                $tag = $obj->getTagName();
                switch ($obj->attr('data-speak')){
                    case 'n':
                        die('Todo Plural');
                        if (isset($plural)) {
                    ...
                    default:
                        switch ($tag){
                            case 'input':
                            case 'textarea':
                                $singular = trim($obj->attr('placeholder'));
                                break;
                            default:
                                $singular = trim($obj->text());
                        }
                        break;
                }
        ...
        while (($tokenCount - $count) > 1) {
            ...
            if (($type == T_STRING) && ($string === $functionName) && ($firstParenthesis === '(')) {
                ...
                if ($mapCount === count($strings)) {
                    ...
                    $this->_addTranslation($domain, $singular, $details);
                } elseif (strpos($this->_file, BIT_CORE_INCLUDE_PATH) === false) {
    ...
    {
        $output = "# LANGUAGE translation of BitPHP Application\n";
    ...
    protected function _searchFiles()
    {
        ...
        foreach ($this->_paths as $path) {
            ...
            $files = $Folder->findRecursive('.*\.(php|ptp|html|thtml|inc|tpl)', true);
            if (!empty($pattern)) {
