...
use Bit\Console\Exception\MissingShellException;
use Bit\Console\Exception\StopException;
use Bit\Core\Bit;
use Bit\Core\Configure;
use Bit\Core\Exception\Exception;
use Bit\Core\Plugin;
use Bit\Log\Log;
use Bit\Shell\Task\CommandTask;
use Bit\Utility\Inflector;

/**
 * Shell dispatcher handles dispatching cli commands.
 *
 * Consult /bin/bit.php for how this class is used in practice.
 */
class ShellDispatcher
{
    ...
    {
        if (!$this->_bootstrap()) {
            $message = "Unable to load BitPHP core.\nMake sure Bit exists in " . BIT_CORE_INCLUDE_PATH;
    ...
    /**
     * Initializes the environment and loads the BitPHP core.
     *
     * @return bool Success.
     */
     ...
     * - `requested` : if used, will prevent the Shell welcome message to be displayed
     * @return bool
     * @throws \Bit\Console\Exception\MissingShellMethodException
     */
     ...
     * @return \Bit\Console\Shell A shell instance.
     * @throws \Bit\Console\Exception\MissingShellException when errors are encountered.
     */
    ...
    protected function _handleAlias($shell)
    {
        ...
        $class = array_map('Bit\Utility\Inflector::camelize', explode('.', $shell));
    ...
    {
        $class = Bit::className($shell, 'Shell', 'Shell');
