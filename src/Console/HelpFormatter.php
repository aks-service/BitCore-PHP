...
class HelpFormatter
{
     ...
     *
     * @param \Bit\Console\ConsoleOptionParser $parser The option parser help is being generated for.
     */
    ...
    public function text($width = 72)
    {
        ...
        if (!empty($subcommands)) {
            ...
            $out[] = '';
            $out[] = sprintf('To see help on a subcommand use <info>`bit %s [subcommand] --help`</info>', $parser->command());
            $out[] = '';
        }
    ...
    {
        $usage = ['bit ' . $this->_parser->command()];
